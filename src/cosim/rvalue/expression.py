...
class ScaledIntRValue(RValue[int]):
    ...
    def __init__(self, coherent: 'cosim.Quint', constant: int):
    ...
    def make_storage_location(self, name: Optional[str] = None):
        return cosim.Quint(cosim.NamedQureg(
...
class QubitIntersection(RValue[bool]):
    ...
    def __init__(self, qubits: Tuple['cosim.Qubit', ...]):
    ...
    def __and__(self, other):
        ...
        if isinstance(other, cosim.Qubit):
            return QubitIntersection(self.qubits + (other,))
        return NotImplemented

    def permutation_registers(self) -> Iterable['cosim.Qureg']:
        return cosim.RawQureg(self.qubits)
    ...
    def __rixor__(self, other):
        if isinstance(other, cosim.Qubit):
            cosim.emit(cosim.OP_TOGGLE(cosim.RawQureg([other])).controlled_by(self))
            return self
        return NotImplemented

    def make_storage_location(self, name: Optional[str] = None):
        return cosim.Qubit(name)

    def init_storage_location(self,
                              location: 'cosim.Qubit',
                              controls: 'cosim.QubitIntersection'):
        cosim.emit(cosim.LetAnd(lvalue=location).controlled_by(self & controls))

    def __str__(self):
        return ' & '.join(str(e) for e in self)

    def __repr__(self):
        return 'cosim.QubitIntersection({!r})'.format(self.qubits)
