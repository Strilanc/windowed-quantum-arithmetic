...
class BoolRValue(RValue[bool]):
    ...
    def __repr__(self):
        return 'cosim.BoolRValue({!r})'.format(self.val)
...
class IntRValue(RValue[bool]):
    ...
    def make_storage_location(self, name: str = ''):
        return cosim.Quint(cosim.NamedQureg(name, self.val.bit_length()))
    ...
    #                          location: 'cosim.Quint',
    #                          controls: 'cosim.QubitIntersection'):
    #     r = cosim.measure_x_for_phase_fixup_and_reset(location)
    #     if cosim.popcnt(r & self.val) & 1:
    #         cosim.phase_flip(controls)
    ...
    def __repr__(self):
        return 'cosim.IntRValue({!r})'.format(self.val)
