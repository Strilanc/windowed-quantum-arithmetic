...
class Mutable(Generic[T]):
    ...
    def __repr__(self):
        return 'cosim.Mutable({!r})'.format(self.val)


class SubEffect:
    def __init__(self, *, op: 'cosim.Operation', args: 'cosim.ArgsAndKwargs'):
        self.op = op
        self.args = args
...
class HeldMultipleRValue:
    def __init__(self,
                 args: cosim.ArgsAndKwargs[SignatureGateArgTypes],
                 name_prefix: str = ''):
        def hold_map(key: Any,
                     e: SignatureGateArgTypes
                     ) -> Tuple[bool,
                                Union['cosim.Operation',
                                      'cosim.HeldRValueManager']]:
            if isinstance(e, cosim.RValue):
                return True, cosim.HeldRValueManager(
                    e,
                    controls=cosim.QubitIntersection.EMPTY,
                    name=name_prefix + str(key))
            return False, e

        self.holders = args.key_map(hold_map)

    def __enter__(self) -> cosim.ArgsAndKwargs[Any]:
        def enter_map(x):
            b, e = x
            if b:
                return e.__enter__()
