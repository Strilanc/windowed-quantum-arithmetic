...
ArgTypes = Union[cosim.RValue[Any], 'cosim.Operation']


@cirq.value_equality
class SignatureOperation(Operation):
    def __init__(self,
                 gate: 'cosim.SignatureGate',
                 args: cosim.ArgsAndKwargs[ArgTypes]):
        self.gate = gate
        self.args = args
    ...
    def mutate_state(self, forward: bool, args: 'cosim.ArgsAndKwargs'):
        matched = args.match_parameters(self.gate.emulate, skip=1)

        def unwrap_immutable(a: cosim.ArgParameter):
            if getattr(a.parameter_type, '__origin__', None) is cosim.Mutable:
                assert isinstance(a.arg, cosim.Mutable)
                return a.arg
            if isinstance(a.arg, cosim.Mutable):
                return a.arg.val
            else:
                return a.arg
    ...
    def do(self, controls: 'cosim.QubitIntersection'):
        with cosim.HeldMultipleRValue(self.args, self.gate.register_name_prefix()) as args:
            try:
    ...
    def __repr__(self):
        return 'cosim.SignatureOperation({!r}, {!r})'.format(
            self.gate, self.args)
