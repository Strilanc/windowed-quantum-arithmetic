...
class LookupTable:
    ...
    def __getitem__(self, item):
        ...
        if isinstance(item, tuple):
            if all(isinstance(e, cosim.Quint) for e in item):
                reg = cosim.RawQureg(q for e in item[::-1] for q in e)
                return LookupRValue(self, cosim.Quint(reg))
        if isinstance(item, cosim.Quint):
            return LookupRValue(self, item)
        return NotImplemented

    def __repr__(self):
        return 'cosim.LookupTable({!r})'.format(self.values)


class XorLookupOperation(Operation):
    def __init__(self,
                 lvalue: 'cosim.Quint',
                 lookup: 'cosim.LookupRValue',
    ...
    def __repr__(self):
        return 'cosim.XorLookupOperation({!r}, {!r}, {!r})'.format(
            self.lvalue,
            self.lookup,
    ...
    def do(self, controls: cosim.QubitIntersection):
        ...
        with cosim.hold(controls & high_bit, name='_lookup_prefix') as q:
...
class LookupRValue(cosim.RValue):
    def __init__(self, table: LookupTable, address: 'cosim.Quint'):
        # Drop high bits that would place us beyond the range of the table.
        max_address_len = cosim.ceil_lg2(len(table))
    ...
    def __rixor__(self, other):
        if isinstance(other, cosim.Quint):
            cosim.emit(XorLookupOperation(other, self))
            return other

        return NotImplemented

    def make_storage_location(self,
                              name: Optional[str] = None) -> 'cosim.Quint':
        return cosim.Quint(cosim.NamedQureg(name, self.table.output_len()))

    def init_storage_location(self,
                              location: 'cosim.Quint',
    ...
    def del_storage_location(self,
        ...
        n = cosim.ceil_lg2(address_count)
        k = min(n >> 1, cosim.floor_lg2(len(location)))
        low = self.address[:k]
        ...
        for i in range(len(location)):
            if cosim.measure_x_for_phase_fixup_and_reset(location[i]):
        ...
        cosim.emit(cosim.LetUnary(lvalue=unary_storage, binary=low))
        cosim.emit(XorLookupOperation(
            lvalue=unary_storage,
            lookup=fixup_table[high],
            phase_instead_of_toggle=True
        ).controlled_by(controls))
        cosim.emit(cosim.LetUnary(lvalue=unary_storage, binary=low).inverse())

    def __str__(self):
        return 'T(len={})[{}]'.format(len(self.table), self.address)

    def __repr__(self):
        return 'cosim.LookupRValue({!r}, {!r})'.format(self.table, self.address)
