namespace FastShor {
    ...
    /// # Summary
    /// Performs a += k where a is a little-endian register and k is a classical constant.
    ///
    /// # Input
    /// ## lvalue
    /// The target of the addition. The 'a' in 'a += k'.
    /// ## offset
    /// The amount to add into the target. The 'k' in 'a += k'.
    operation PlusEqualConst (lvalue: LittleEndian, offset: Int) : Unit {
        body (...) {
            Controlled PlusEqualConst(new Qubit[0], (lvalue, offset));
        }
        adjoint auto;
        controlled (cs, ...) {
            if (USE_SHORTCUTS()) {
                Controlled IntegerIncrementLE(cs, (offset, lvalue));
            } else {
                using (t_reg = Qubit[Length(lvalue!)]) {
                    let t = LittleEndian(t_reg);
                    Controlled LetConst(cs, (t, offset));
                    PlusEqual(lvalue, t);
                    Controlled DelConst(cs, (t, offset));
                }
            }
        }
    ...
    /// # Input
    /// ## lvalue
    /// The target of the subtraction. The 'a' in 'a -= k'.
    /// ## offset
    /// The amount to subtract out of the target. The 'k' in 'a -= k'.
    operation MinusEqualConst (lvalue: LittleEndian, offset: Int) : Unit {
        body (...) {
            Adjoint PlusEqualConst(lvalue, offset);
        }
        adjoint auto;
        controlled auto;
        controlled adjoint auto;
